...
%                           %of I2.
         %LBP代码图像使用SP中的采样点并且没有映射。现在H2等于I2的直方图。
...
% Block size, each LBP code is computed within a block of size bsizey*bsizex 块大小，每个LBP代码在大小为bsizey * bsizex的块内计算
...
for i = 1:neighbors
  ...
  if (abs(x - rx) < 1e-6) && (abs(y - ry) < 1e-6)
    ...
    % Calculate the interpolation weights.计算插值权重
    w1 = (1 - tx) * (1 - ty);
    w2 =      tx  * (1 - ty);
    w3 = (1 - tx) *      ty ;
    w4 =      tx  *      ty ;
    % Compute interpolated pixel values计算插值像素值
...
if isstruct(mapping)
    ...
    for i = 1:size(result,1)
        for j = 1:size(result,2)
            result(i,j) = mapping.table(min(255,result(i,j)+1));
        end
    end
end
